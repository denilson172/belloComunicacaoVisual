<?php
	...
	$financa_controller->encontrarFinanca($_GET['id']);

	if(empty($_SESSION['financaEncontrada'])){
		echo "erro";
	}else{
		$financaEncontrada = $_SESSION['financaEncontrada'];
	}
?>
...
<!DOCTYPE html>
	...
	<head>
		...
		<title>Bello - Editar Finanças</title>
	...
	<body class="blue-bg">
		...
		<!--========inicio detalhes=====================================================-->
		<?php
			foreach ($financaEncontrada as $fin){
	...
	echo				'<form action="../../../Controller/financasController.php"" method="POST">';
	echo					'<div class="modal-header">';
	echo						'<h4 class="modal-title">Editar Finança | Cód: ' .$idFinancas.'</h4>';
	echo					'</div>';
	echo						'<div class="modal-body">';
	echo						'<div class="form-group">';
	echo							'<label>Data de Inserção</label>';
	echo							'<input type="text" name="data" class="form-control" value ="'.$dataFinancas.'" required>';							
	echo						'</div>';
	echo						'<div class="form-group">';
	echo							'<label>Categoria</label>';
	echo							'<input type="text" name="categoria" class="form-control" value ="'.$categoriaFinancas.'" required>';	
	echo						'</div>';
	echo						'<div class="form-group">';
	echo							'<label>Descrição</label>';
	echo							'<input type="text" name="descricao" class="form-control" value ="'.$descricaoFinancas.'" required>';	
	echo						'</div>';
	echo						'<div class="form-group">';
	echo							'<label>Valor</label>';
	echo							'<input type="text" name="valor" class="form-control" value ="'.$valorFinancas.'" required>';	
	echo						'</div>';
	echo						'<div class="form-group">';
	echo							'<label>Tipo de Financa</label>';
	echo							'<input type="text" name="tipoFinanca" class="form-control" value ="'.$tipoFinancaFinancas.'" disabled>';	
	echo						'</div>';
	echo						'<div class="form-group">';
	echo							'<label>Armazenamento</label>';
	echo							'<input type="text" name="armazenamento" class="form-control" value ="'.$armazenamentoFinancas.'" required>';	
	echo						'</div>';
	echo					'</div>';
	echo					'<div class="modal-footer">';
	echo					'<input type="hidden" name="classe" value="Financas">';
	echo					'<input type="hidden" name="metodo" value="alterarFinancas">';
	echo						'<input type="button" class="btn btn-danger" value="Cancelar">';
